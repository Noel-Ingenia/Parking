...
module.exports = {
    ...
    homeGetGroupReleasedDays: 'SELECT  P.ParkingName, P.MapUrl, Pl.PlaceName, RD.SelectedDay ' +
    'FROM Place Pl, Parking P, ReleasedDays RD, Released R, Assignation A, Occupant O ' +
    'WHERE P.Id = Pl.IdParking AND Pl.Id = A.IdPlace AND O.IdGroup = @paramInt AND A.IdOccupant = O.Id AND R.IdAssignation = A.Id ' +
    'AND RD.IdReleased = R.Id',
    ...
    'AND RQD.IdReleasedDay = RD.Id AND RQ.Id = RQD.IdRequest AND RQ.IdEmployee = @paramInt AND ' +
    ...
    'FROM Place Pl, Parking P, ReleasedDays RD, Released R, Assignation A, Occupant O ' + 
    'WHERE P.Id = Pl.IdParking AND Pl.Id = A.IdPlace AND O.IdUser = @paramInt ' +
    ...
    'FROM Place Pl, Parking P, ReleasedDays RD, Released R, Assignation A, Occupant O ' + 
    'WHERE P.Id = Pl.IdParking AND Pl.Id = A.IdPlace AND O.IdUser = @paramInt AND A.IdOccupant = O.Id ' +
